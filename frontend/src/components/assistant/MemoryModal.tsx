...
export const MemoryModal: React.FC<MemoryModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#030712]/85 backdrop-blur-md flex items-center justify-center p-4 select-none">
      <div className="bg-panel border border-white/[0.1] rounded-3xl w-full max-w-lg p-6 space-y-5 shadow-2xl relative animate-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted">Manage facts, preferences, and household rules</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-xl bg-secondary text-muted hover:text-white transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {/* Add Memory Form */}
        <form onSubmit={handleAddMemory} className="space-y-3 bg-background/60 p-3.5 rounded-2xl border border-primary">
          ...
          <div className="flex gap-2">
            <select
              ...
              className="bg-panel border border-primary rounded-xl px-2.5 py-2 text-xs text-secondary focus:outline-none focus:border-blue-500"
            >
            ...
            <input
              ...
              className="flex-1 bg-panel border border-primary rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500"
            />
            <button
              type="submit"
        ...
        <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1 scrollbar-thin">
          {loading ? (
            ...
            memories.map((m) => (
              <div
                key={m.id}
                className="p-3 rounded-2xl bg-background/70 border border-white/[0.05] hover:border-primary flex items-start justify-between gap-3 group transition-colors"
              >
                ...
                <button
                  ...
                  className="text-muted hover:text-rose-400 p-1 rounded-lg opacity-80 group-hover:opacity-100 transition-all shrink-0"
                  title="Delete memory"
                >
        ...
        <div className="pt-2 border-t border-primary text-[10px] text-muted flex items-center gap-1.5">
